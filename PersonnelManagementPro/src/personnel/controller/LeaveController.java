...
public class LeaveController {
	...
	public String toleaveadd(Model model) {
		...
		//查询出请假类型
		List<LeaveType>  leavetype_list = personnelservice.findLeaveType();
	...
	public void leaveadd(ModelAndView mv,@RequestParam("leavedays")Integer leavedays,
		...
		int emp_id = personnelservice.getEmpIdByUserId(id);
		
		Employee employee = personnelservice.get_EmployeeInfo(emp_id);
	    ...
	    //3.执行添加操作
		personnelservice.insert_LeaveInfo(leave);
	...
	public String leavelist(Integer id,Model model) {
		...
		//1.通过传入的当前用户id，查出员工姓名。
		int emp_id = personnelservice.getEmpIdByUserId(id); //唯一的员工id
		
		List<Leave> leave_list = personnelservice.findLeaveListById(emp_id);
		
		/*if (content!=null&&!content.equals("")){
			leave_list = ahualyService.findLeaveListByIdAndCon(content,id);
		}*/
		
		Integer count = personnelservice.countLeave(emp_id);
	...
	public String leavedelete(Integer id) {
		...
		personnelservice.delete_LeaveInfo(id);
	    ...
	    Leave leave = personnelservice.findLeaveById(id);
		...
		public ModelAndView leaveedit(ModelAndView mv,@RequestParam("leavedays")Integer leavedays,
			...
			this.genericAssociation(job_id,dept_id,leavetype_id,leave);
		    //3.执行更新操作
			personnelservice.update_LeaveInfo(leave);
			//4.添加成功后重定向到我的请假页面
	    ...
	    public String adminleavelist(Integer pageIndex,Model model,String content,Leave leave) {
			...
			List<Leave> job_list = personnelservice.get_LeaveList(leave,pageModel);
			
			//按照关键字content查询
			Integer count = 0;
			if (content!=null&&!content.equals("")){
				count = personnelservice.countAllLeave(content);
				job_list = personnelservice.get_LeaveLikeList(content);
	  ...
	  //管理员跳转请假编辑页面
	    ...
	    //查询出请假类型
	    List<LeaveType>  leavetype_list = personnelservice.findLeaveType();
	  	List<LeaveStatus> leavestatus_list = personnelservice.findLeaveStatus();
		...
		public ModelAndView adminleaveedit(ModelAndView mv,@RequestParam("leavedays")Integer leavedays,
			...
			Integer emp_id = personnelservice.getEmpIdById(leave_id);
			User user = personnelservice.getStaticId(emp_id);
