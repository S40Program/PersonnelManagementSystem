...
public class DeptController {
	...
	@RequestMapping(value="/dept/list",method=RequestMethod.GET)
	 public String index(Integer pageIndex,Model model,String content,Dept dept){
		...
		if (content!=null&&!content.equals("")){
			dept_list = personnelservice.findAllDept(content);
		}
	...
	@RequestMapping(value="/dept/add",method=RequestMethod.GET)
	 public String add(Model model,Integer id){
		if(id!=null){
			Dept dept = personnelservice.get_Info(id);
	...
	@RequestMapping(value="/dept/add",method=RequestMethod.POST)
	 public ModelAndView add(ModelAndView mv,@ModelAttribute Dept dept ,Integer id){
		if(id!=null){
			personnelservice.update_Info(dept);
		}else{
			//首先得确定没有此部门，才能做添加工作
			Dept d = personnelservice.get_deptByname(dept.getName());
			if(d==null) {
				personnelservice.addDept(dept);
	...
	@RequestMapping(value="/dept/delete",method=RequestMethod.GET)
	 public void delete(Integer id){
		...
		if(id!=null){
			personnelservice.delete_Info(id);
		}
	}
}
