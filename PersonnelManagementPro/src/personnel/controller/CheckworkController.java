	   ...
	   //如果在目录下输入为空，则跳转到指定链接
		...
		@RequestMapping(value="/checkwork/list",method=RequestMethod.GET)
		 public String index(Integer pageIndex,Model model,String content,Checkwork checkwork){
			...
			List<Checkwork> job_list = personnelservice.get_CheckworkList(checkwork,pageModel);
			Integer count = 0;
			if (content!=null&&!content.equals("")){
				count = personnelservice.countCheckwork(content);
				job_list = personnelservice.get_CheckworkLikeList(content);
			}
		...
		@RequestMapping(value="/checkwork/add",method=RequestMethod.POST)
		 public ModelAndView add(ModelAndView mv,
			...
			if(id!=null){
				...
				personnelservice.update_CheckworkInfo(checkwork);
				...
				if(employee_id!=null&&personnelservice.get_CheckworkEmp_id(employee_id)==null) {
						...
						personnelservice.insert_CheckworkInfo(checkwork);
		...
		@RequestMapping(value="/checkwork/delete",method=RequestMethod.GET)
		 public void delete(Integer id){
			...
			if(id!=null){
				personnelservice.delete_Checkwork(id);
			}
		}
