	...
	private PersonnelService personnelservice;
		...
		@RequestMapping(value="/contract/list",method=RequestMethod.GET)
		 public String index(Integer pageIndex,Model model,String content,Contract contract){
			...
			List<Contract> job_list = personnelservice.get_ContractList(contract,pageModel);
			Integer count = 0;
		    if(content!=null&&!content.equals("")){
			count = personnelservice.countContract(content);
			job_list = personnelservice.get_ContractLikeList(content);
			}
		...
		@RequestMapping(value="/contract/add",method=RequestMethod.POST)
		 public ModelAndView add(ModelAndView mv,
			...
			if(id!=null){
				...
				personnelservice.update_ContractInfo(contract);
			}else{
				employee_id = personnelservice.get_EmployeeByName(employee_name);
				if(employee_id!=null&&personnelservice.get_ContractEmp_id(employee_id)==null) {
					 //根据employee_id去查询员工的信息，前端只需要添加合同即可。
				  ...
				  //只需要将id设置为员工信息的id（即全局id即可）
					...
					personnelservice.insert_ContractInfo(contract);
		...
		@RequestMapping(value="/contract/delete",method=RequestMethod.GET)
		 public void delete(Integer id){
			...
			if(id!=null){
				personnelservice.delete_Contract(id);
			}
		}
