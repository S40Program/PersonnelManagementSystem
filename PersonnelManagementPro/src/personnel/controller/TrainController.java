...
public class TrainController {
	...
	public String trainlist(Integer id,Model model) {
				...
				List<Train> train_list = personnelservice.findTrainListById(emp_id);

				
				Integer count = personnelservice.countTrain(emp_id);
	...
	public String admintrainlist(Integer pageIndex,Model model,String content,Train train) {
		...
		List<Train> train_list = personnelservice.get_TrainList(train,pageModel);
			Integer count = 0;
			if (content!=null&&!content.equals("")){
				count = personnelservice.countAllTrain(content);
				train_list = personnelservice.get_TrainLikeList(content);
			}
			model.addAttribute("count",count);
			model.addAttribute("train_list",train_list);
	...
	public String  trainadd(Model model) {
		//需要查询出一些固定数据
		List<Completion> completion_list = personnelservice.findCompletion();
	   ...
	   //管理员培训添加提交  
		...
		public ModelAndView  totrainadd(ModelAndView mv,@RequestParam("employee_name")String employee_name,@RequestParam("content") String content,
			...
			//保证有这个员工就ok
			Integer employee_id = personnelservice.get_EmployeeByName(employee_name);
			  ...
			  if(employee_id!=null){
				  //根据employee_id去查询员工的信息，前端只需要添加培训信息即可。
				    ...
				    personnelservice.insert_TrainInfo(train);
	   ...
	   //管理员培训编辑页面跳转  
		...
		public String  trainedit(Integer id,Model model) {
			...
			Train train = personnelservice.findTrainById(id);
		...
		public ModelAndView  totrainedit(ModelAndView mv,@RequestParam("content") String content,
			...
			personnelservice.update_TrainInfo(train);
		    mv.setViewName("redirect:/train/admintrainlist");
		...
		public void delete(Integer id) {
			personnelservice.delete_Train(id);
		}
