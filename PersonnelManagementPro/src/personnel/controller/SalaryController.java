	   ...
	   //如果在目录下输入为空，则跳转到指定链接
		...
		@RequestMapping(value="/salary/list",method=RequestMethod.GET)
		 public String index(Integer pageIndex,Model model,String content,Salary salary){
			...
			List<Salary> job_list = personnelservice.get_SalaryList(salary,pageModel);
			Integer count = 0;
			if (content!=null&&!content.equals("")){
				count = personnelservice.countSalary(content);
				job_list = personnelservice.get_SalaryLikeList(content);
			}
		...
		@RequestMapping(value="/salary/add",method=RequestMethod.POST)
		 public ModelAndView add(ModelAndView mv,Integer job_id,Integer dept_id,String employee_name,@ModelAttribute Salary salary ,Integer id
			...
			if(id!=null){
				...
				personnelservice.update_SalaryInfo(salary);
			}else{
				 employee_id = personnelservice.get_EmployeeByName(employee_name);
				  if(employee_id!=null&&personnelservice.get_SalaryEmp_id(employee_id)==null){
					  ...
					  //只需要将id设置为员工信息的id（即全局id即可）
					    ...
					    personnelservice.insert_SalaryInfo(salary);
		...
		@RequestMapping(value="/salary/delete",method=RequestMethod.GET)
		 public void delete(Integer id){
			...
			if(id!=null){
				personnelservice.delete_Salary(id);
			}
		}
