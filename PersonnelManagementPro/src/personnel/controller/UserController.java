	...
	private PersonnelService personnelservice;//接口实现类对象
		...
		@RequestMapping(value="/user/list",method=RequestMethod.GET)
		 public String index(Integer pageIndex,Model model,String content,User user){
			...
			if (content!=null&&!content.equals("")){
				count = personnelservice.countUser(content);
				user_list = personnelservice.get_UserLikeList(content);
			}
		...
		@RequestMapping(value="/user/add",method=RequestMethod.POST)
		 public ModelAndView add(ModelAndView mv,@ModelAttribute User user ,Integer id){
			    ...
			    User user2 = personnelservice.findUserByLogin(user.getLoginname());
			    User user3 = personnelservice.findUserByName(user.getUsername());
			    if(user1==null&&user2==null&&user3==null) {
			    	//检查邮箱是否已经被注册
			    	//通过用户信息提交的邮箱号去user_inf表中去查询是否有邮箱所对应的用户
			    	User user4 = personnelservice.findUserByEmail(user.getEmail());
			    	if(user4==null) {
			    	//对明文密码进行加密
				    user.setPassword(ShiroMD5Privacy.privacy(user.getLoginname(), user.getPassword()));
				  //这个地方需要生成一个id，这个id是为了给che_id,sal_id,con_id,ins_id提供的一个值
				    //那这个id该如何去生成，怎么就能唯一呢
		...
		@RequestMapping(value="/user/delete",method=RequestMethod.GET)
		 public void delete(Integer id){
			...
			if(id!=null){
				personnelservice.delete_UserInfo(id);
			}
		}
