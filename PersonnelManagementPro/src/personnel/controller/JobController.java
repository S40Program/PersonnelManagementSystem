	   ...
	   //如果在目录下输入为空，则跳转到指定链接
		...
		@RequestMapping(value="/job/list",method=RequestMethod.GET)
		 public String index(Integer pageIndex,Model model,String content,Job job){
			...
			if (content!=null&&!content.equals("")){
				job_list = personnelservice.findAllJob(content);
			}
		...
		@RequestMapping(value="/job/add",method=RequestMethod.GET)
		 public String add(Model model,Integer id){
			if(id!=null){
				Job job = personnelservice.get_JobInfo(id);
		...
		@RequestMapping(value="/job/add",method=RequestMethod.POST)
		 public ModelAndView add(ModelAndView mv,@ModelAttribute Job job ,Integer id){
			...
			if(id!=null){
				personnelservice.update_JobInfo(job);
			}else{
				//首先判断是否已经存在这个工作岗位
				Job j = personnelservice.get_jobByname(job.getName());
				if(j==null) {
					personnelservice.insert_JobInfo(job);
		...
		@RequestMapping(value="/job/delete",method=RequestMethod.GET)
		 public void delete(Integer id){
			System.out.println(id);
			if(id!=null){
				personnelservice.delete_JobInfo(id);
			}
		}
}
