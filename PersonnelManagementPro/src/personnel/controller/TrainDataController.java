...
import personnel.pojo.TrainData;
	   ...
	   //如果在目录下输入为空，则跳转到指定链接
		...
		@RequestMapping(value="/traindata/list",method=RequestMethod.GET)
		 public String index(Integer pageIndex,Model model,String content,TrainData trainData){
			...
			List<TrainData> job_list = personnelservice.get_TrainDataList(trainData,pageModel);
			Integer count = 0;
			if (content!=null&&!content.equals("")){
				count = personnelservice.countTrainData(content);
				job_list = personnelservice.get_TrainDataLikeList(content);
			}
		...
		@RequestMapping(value="/traindata/add",method=RequestMethod.GET)
		 public String add(Model model,Integer id){
			if(id!=null){
				TrainData job = personnelservice.get_TrainDataInfo(id);
		...
		@RequestMapping(value="/traindata/add",method=RequestMethod.POST)
		 public ModelAndView add(ModelAndView mv,@ModelAttribute TrainData trainData ,Integer id,HttpSession session
			...
			if(id!=null){
				...
				personnelservice.update_TrainDataInfo(trainData);
			}else{
				/**
				 * 上传文件
				 */
				...
				personnelservice.insert_TrainDataInfo(trainData);
			}
		...
		@RequestMapping(value="/traindata/delete",method=RequestMethod.GET)
		 public void delete(Integer id){
			...
			if(id!=null){
				personnelservice.delete_TrainDataInfo(id);
			}
		}
		...
		@RequestMapping(value="/traindata/downLoad")
		 public ResponseEntity<byte[]>  downLoad(
			...
			// 根据id查询文档
			TrainData target = personnelservice.get_TrainDataInfo(id);
