...
import personnel.pojo.Document;
	   ...
	   //如果在目录下输入为空，则跳转到指定链接
		...
		@RequestMapping(value="/document/list",method=RequestMethod.GET)
		 public String index(Integer pageIndex,Model model,String content,Document document){
			...
			List<Document> job_list = personnelservice.get_DocumentList(document,pageModel);
			Integer count = 0;
			if (content!=null&&!content.equals("")){
				count = personnelservice.countDocument(content);
				job_list = personnelservice.get_DocumentLikeList(content);
			}
		...
		@RequestMapping(value="/document/add",method=RequestMethod.GET)
		 public String add(Model model,Integer id){
			if(id!=null){
				Document job = personnelservice.get_DocumentInfo(id);
		...
		@RequestMapping(value="/document/add",method=RequestMethod.POST)
		 public ModelAndView add(ModelAndView mv,@ModelAttribute Document document ,Integer id,HttpSession session
			...
			if(id!=null){
				...
				personnelservice.update_DocumentInfo(document);
			}else{
				/**
				 * 上传文件
				 */
				...
				personnelservice.insert_DocumentInfo(document);
			}
		...
		@RequestMapping(value="/document/delete",method=RequestMethod.GET)
		 public void delete(Integer id){
			...
			if(id!=null){
				personnelservice.delete_DocumentInfo(id);
			}
		}
		...
		@RequestMapping(value="/document/downLoad")
		 public ResponseEntity<byte[]>  downLoad(
			...
			// 根据id查询文档
			Document target = personnelservice.get_DocumentInfo(id);
